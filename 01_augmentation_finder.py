...
def get_split(random_state, df_data):
    ...
    """
    # First split: 9-fold stratified group split
    ...
    sgkf_val_test = StratifiedGroupKFold(n_splits=5, shuffle=True, random_state=random_state)
...
def objective(trial, train_dataset, val_dataset):
    ...
    for epoch in range(int(dynamic_args.num_train_epochs)):
        trainer.train(resume_from_checkpoint=None)
        metrics = trainer.evaluate()
        val_uar = metrics["eval_uar"]

        # report al pruner
        trial.report(val_uar, step=epoch)
        # pruning check
        if trial.should_prune():
            # cleanup prima di interrompere
            del model, trainer
...
best_params_df["best_value"] = study.best_value  # opzionale
